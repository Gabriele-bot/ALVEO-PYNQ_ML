# This is a generated file. Use and modify at your own risk.
################################################################################
  # This constraints file contains default clock frequencies to be used during
# out-of-context flows such as OOC Synthesis and Hierarchical Designs. For
# best results the frequencies should be modified to match the target
# frequencies. This constraints file is not used in normal top-down
# synthesis (the default flow of Vivado)
create_clock -period 4.000 [get_ports ap_clk]
# bscan clocks defined
create_clock -name bscan_0_update -period 60.0 [get_ports bscan_0_update]
create_clock -name bscan_0_drck   -period 30.0 [get_ports bscan_0_drck]
create_clock -name bscan_0_tck    -period 30.0 [get_ports bscan_0_tck]

